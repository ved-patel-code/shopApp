...
export function AddToCreditForm({
  ...
  const [printBill, setPrintBill] = useState(false); // <-- NEW state for the checkbox
  ...
  useEffect(() => {
    ...
    }, 200); // 200ms delay as you specified
  ...
  const handleAddToCredit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: errorMessage, // Now this will always be a string
      });
    } finally {
      setIsCheckingOut(false);
    }
